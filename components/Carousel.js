...
export default class Carousel extends Component {
    ...
    _onScrollBegin = (event) => {
        this._clearTimer();
        this.startOffset = event.nativeEvent.contentOffset.x;
    }

    _onScrollEnd = (event) => {
        const offset = { ...event.nativeEvent.contentOffset };
        this.endOffset = event.nativeEvent.contentOffset.x;
        const page = this._calculateCurrentPage(offset.x);
        const direction = this.startOffset - this.endOffset > 0 ? "right" : "left";
        this._placeCritical(page);
        this._setCurrentPage(page);
        if (this.state.currentPage !== page && typeof this.props.onPageChanged === "function")
            this.props.onPageChanged(page, direction);
        this._setUpTimer();
    }
    ...
    _onScroll = (event) => {
        ...
        if (this.nextPage !== nextPage) {
            ...
            if (this.props.onPageBeingChanged) {
                this.props.onPageBeingChanged(this.nextPag, direction);
            }
        }
    }
